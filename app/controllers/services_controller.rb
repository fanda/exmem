class ServicesController < ApplicationController
  ...
  def create
    ...
    service_route = if params[:service]
      params[:service]
    else
      'No service recognized (invalid callback)'
    end
    ...
    if omniauth and service_route = params[:service]
      ...
      and user_info = omniauth['info']
          user_info['email'] ? @authhash[:email] =  user_info['email'] : @authhash[:email] = ''
          user_info['name'] ? @authhash[:name] =  user_info['name'] : @authhash[:name] = ''
      ...
      unless @authhash[:uid].blank? or @authhash[:provider].blank?
        auth = Service.find_by_provider_and_uid(@authhash[:provider], @authhash[:uid])
        # if the user is currently signed in, he might want to add another account to signin
        if user_signed_in?
          if auth
